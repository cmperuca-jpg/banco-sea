...
const runtimeRoot = path.resolve(process.env.FUSION_RUNTIME_ROOT || (isRender ? persistentRoot : __dirname));
const dataRoot = path.join(runtimeRoot, "data");
const uploadsRoot = path.join(runtimeRoot, "uploads");
// No aplicativo instalado, os arquivos visuais ficam fora do ASAR para que
// Express entregue CSS, JavaScript, imagens e fontes como arquivos normais.
const publicRoot = path.resolve(
  process.env.FUSION_PUBLIC_ROOT || path.join(__dirname, "public")
);
...
function prepararPersistenciaRender() {
  if (!isRender) {
    garantirDiretorio(dataRoot);
    garantirDiretorio(uploadsRoot);
    if (runtimeRoot !== __dirname) {
      copiarSeedsSeDiretorioVazio(path.join(__dirname, "data"), dataRoot);
      copiarSeedsSeDiretorioVazio(path.join(__dirname, "uploads"), uploadsRoot);
    }
    return;
  }
...
const pagamentosJsonCandidates = [
  path.join(dataRoot, "financeiro", "pagamentos.json"),
  path.join(dataRoot, "pagamentos.json"),
  path.join(runtimeRoot, "database", "financeiro", "pagamentos.json"),
  path.join(runtimeRoot, "database", "pagamentos.json"),
  path.join(runtimeRoot, "db", "financeiro", "pagamentos.json"),
  path.join(runtimeRoot, "db", "pagamentos.json")
];
...
app.get("/api/health", (req, res) => {
  const dataPath = dataRoot;
  const uploadsPath = uploadsRoot;
  res.json({
    ok: true,
    sistema: "Fusion ERP",
    versao: "2.8.0-piloto",
...
app.use("/uploads", protegerUploads, express.static(uploadsRoot, {
  dotfiles: "deny",
  fallthrough: false,
  index: false,
  setHeaders(res) {
...
app.get("/api/sistema/diagnostico", async (req, res) => {
  ...
  const arquivos = arquivosObrigatorios.map((relativo) => {
    const absoluto = path.join(runtimeRoot, relativo);
...
app.get("/api/sistema/interface", async (req, res) => {
  ...
  const paginas = paginasCriticas.map((relativo) => {
    const absoluto = path.join(publicRoot, "pages", relativo);
    const existe = fs.existsSync(absoluto);
    let bytes = 0;
...
export const fusionHttpServer = app.listen(PORT, HOST, async () => {
  console.log("====================================");
...
export async function fecharFusionServidor() {
  try { await encerrarPersistenciaSupabase(); }
  catch (erro) { console.error(`[Persistência] Falha no encerramento: ${erro.message}`); }
  if (fusionHttpServer?.listening) {
    await new Promise((resolve) => fusionHttpServer.close(() => resolve()));
  }
}

async function encerrarServidor(sinal) {
  console.log(`[Sistema] ${sinal}: encerrando o servidor local.`);
  await fecharFusionServidor();
  process.exit(0);
}

if (!process.env.FUSION_DESKTOP_EMBEDDED) {
