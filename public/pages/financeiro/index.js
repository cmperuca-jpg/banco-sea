...
function renderizarTabela() {
  if (els.contadorLancamentos) {
    const total = lancamentos.length;
    els.contadorLancamentos.textContent = `${total.toLocaleString("pt-BR")} ${total === 1 ? "lançamento" : "lançamentos"}`;
  }

  if (!lancamentos.length) {
    els.tabela.innerHTML = `<tr><td colspan="11"><div class="fusion-ui-empty"><strong>Nenhum lançamento encontrado</strong><span>Ajuste os filtros ou cadastre um novo lançamento financeiro.</span></div></td></tr>`;
    return;
  }
  els.tabela.innerHTML = lancamentos.map((item) => {
    ...
    return `<tr>
      ...
      <td><div class="acoes">
        <button type="button" class="btn-secondary" onclick="editarLancamento('${escapeHtml(item.id)}')">Editar</button>
        <button type="button" class="btn-light" ${jaPago ? "disabled" : ""} onclick="baixarLancamento('${escapeHtml(item.id)}')">Baixar</button>
        <button type="button" class="btn-danger" onclick="excluirLancamento('${escapeHtml(item.id)}')">Excluir</button>
      </div></td>
    </tr>`;
  }).join("");
}
