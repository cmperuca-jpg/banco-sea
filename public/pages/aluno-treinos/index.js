...
function rotaAluno(nome, fallback) {
  return CONFIG.rotas?.[nome] || fallback;
}

function rotaComAluno(nome, fallback) {
  const sessao = sessaoAluno();
  const base = rotaAluno(nome, fallback);
  if (!sessao?.alunoId) return base;
  return `${base}${base.includes("?") ? "&" : "?"}alunoId=${encodeURIComponent(sessao.alunoId)}`;
}
...
function exigirLogin() {
  const sessao = sessaoAluno();
  if (!sessao?.alunoId) {
    location.replace(rotaAluno("login", "/pages/aluno-login/index.html"));
    return null;
  }
  return sessao;
}
...
function abrirAvaliacao() {
  ...
  location.href = rotaComAluno("avaliacao", "/pages/aluno-avaliacao/index.html");
}
...
const portalInicio = rotaComAluno("inicio", "/pages/portal-aluno-emergencial/index.html");
if ($("linkPortalInicio")) $("linkPortalInicio").href = portalInicio;
if ($("btnPortalInicio")) $("btnPortalInicio").href = portalInicio;
