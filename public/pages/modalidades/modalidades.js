...
const elementos = {
  ...
  mensagem: document.getElementById("mensagemSincronizacao"),
  btnSincronizar: document.getElementById("btnSincronizarSupabase")
};
...
function mostrarMensagem(texto, tipo = "sucesso") {
  ...
  elementos.mensagem.className = `mensagem-sincronizacao visivel ${tipo}`;
}
...
window.excluirModalidade = async function (id) {
  ...
  try {
    const payload = await requestJson(`${API}/${id}`, { method: "DELETE" });
    mostrarMensagem(
      payload.mensagem || "Modalidade excluída com sucesso.",
      payload.nuvem?.ok === false ? "aviso" : "sucesso"
    );
...
async function sincronizarModalidadesSupabase() {
  if (!confirm("Enviar todas as modalidades locais para o novo Supabase?")) return;

  const botao = elementos.btnSincronizar;
  const textoOriginal = botao?.textContent || "Sincronizar nuvem";
  try {
    if (botao) {
      botao.disabled = true;
    ...
    const payload = await requestJson(`${API}/sincronizar-supabase`, { method: "POST" });
    mostrarMensagem(`${payload.enviados || 0} modalidade(s) sincronizada(s) com sucesso.`, "sucesso");
  } catch (error) {
    mostrarMensagem(`Falha na sincronização: ${error.message}`, "erro");
  } finally {
    if (botao) {
...
elementos.form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    mostrarMensagem(
      resultado.mensagem || (id ? "Modalidade atualizada com sucesso." : "Modalidade cadastrada com sucesso."),
      resultado.nuvem?.ok === false ? "aviso" : "sucesso"
    );
    fecharModal();
    await carregarModalidades();
  } catch (error) {
...
elementos.btnSincronizar?.addEventListener("click", sincronizarModalidadesSupabase);
...
carregarModalidades().catch(error => mostrarMensagem(error.message, "erro"));
