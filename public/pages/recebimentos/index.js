...
function valorRecebidoBruto(item){return Number(campoValor(item,["valorRecebido","recebido","valorPago"],0))}
function taxaItem(item){return Number(campoValor(item,["taxaOperadoraValor","taxaValor","taxa"],0))}
function valorRecebido(item){const liquido=Number(campoValor(item,["valorLiquido","liquido","valorRecebidoLiquido"],NaN));if(Number.isFinite(liquido)&&liquido>0)return liquido;return Math.max(0,Number((valorRecebidoBruto(item)-taxaItem(item)).toFixed(2)))}
function saldoItem(item){const s=campoValor(item,["valorRestante","saldo","valorAberto"],null);return s!==null?Number(s):Math.max(0,valorBruto(item)-valorRecebidoBruto(item))}
...
function textoDias(item){if(!estaEmAberto(item))return{texto:"-",classe:""};const d=diferencaDias(dataItem(item));if(d===null)return{texto:"-",classe:""};if(d<0)return{texto:`${Math.abs(d)} dia${Math.abs(d)===1?"":"s"}`,classe:"atrasado"};if(d===0)return{texto:"Hoje",classe:"hoje"};return{texto:"-",classe:"futuro"}}
...
function erroAmigavel(e){const msg=String(e?.message||"Erro inesperado");if(e?.status===404)return"Rota não encontrada. Confirme se o módulo financeiro do servidor está ativo.";if(msg.includes("Failed to fetch"))return"Não foi possível conectar ao servidor local. Verifique se o Fusion Sistema está iniciado.";return msg}
