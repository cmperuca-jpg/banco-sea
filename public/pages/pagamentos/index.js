...
function imprimirResumo(){
  ...
  if(!w)return;
  w.document.write(`<html><head><title>Resumo de Pagamentos</title><style>body{font-family:Arial;padding:24px}h1{color:#22b8d2}.summary-box{border:1px solid #ddd;margin:12px 0;padding:12px;border-radius:8px}table{width:100%;border-collapse:collapse}td,th{border-bottom:1px solid #ddd;padding:7px;text-align:left}.num{text-align:right}.summary-total{font-size:22px;font-weight:bold;color:#22b8d2}</style></head><body><h1>Fusion Sistema</h1><h2>Resumo de Pagamentos</h2>${conteudo}<p>Emitido em ${new Date().toLocaleString("pt-BR")}</p><script>window.print();setTimeout(()=>window.close(),700)</script></body></html>`);
  w.document.close();
}
