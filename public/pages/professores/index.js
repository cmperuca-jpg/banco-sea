...
function formatarCpf(v){const n=somenteNumeros(v).slice(0,11);return n.replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d{1,2})$/,'$1-$2');}
function senhaAcesso(p={}){return String(p.senhaAcesso||p.senhaPortal||p.senha||'');}
...
function filtrados(){
  ...
  const qNum=somenteNumeros($('#busca').value);
  ...
  return professores.filter(p=>{
    ...
    return (!q||alvo.includes(q)||(qNum&&somenteNumeros(p.cpf).includes(qNum)))&&(!st||status(p)===st);
  });
}
...
function resetarCamposSenha(valor=''){
  const senha = $('#senha');
  const confirmar = $('#confirmarSenha');
  [senha, confirmar].forEach(el => {
    if (!el) return;
    el.value = valor;
    el.dataset.editado = '';
  });
...
async function salvar(ev){ev.preventDefault();const id=$('#id').value;const senhaEl=$('#senha');const confirmarEl=$('#confirmarSenha');const senhaEditada=!id||senhaEl?.dataset.editado==='true'||confirmarEl?.dataset.editado==='true';const senha=senhaEditada?(senhaEl?.value||''):'';const confirmar=senhaEditada?(confirmarEl?.value||''):'';const dados=coletar();if(!dados.nome||dados.nome.length<3)return mostrar('Informe o nome completo.','erro');if(!id&&!senha)return mostrar('Informe a senha do Portal do Professor.','erro');if(senhaEditada&&senha&&senha.length<6)return mostrar('A senha deve ter pelo menos 6 caracteres.','erro');if(senhaEditada&&senha!==confirmar)return mostrar('A confirmação da senha não confere.','erro');const btn=$('#btnSalvar');try{if(btn){btn.disabled=true;btn.textContent='Salvando...';}const resp=await fetch(id?`${API}/${encodeURIComponent(id)}`:API,{method:id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(dados)});const j=await safeJson(resp);if(!resp.ok||j.ok===false)throw new Error(j.mensagem||j.erro||`HTTP ${resp.status}`);fechar();mostrar(j.mensagem|| (id?'Professor atualizado.':'Professor cadastrado.'),'sucesso');await carregar();}catch(e){mostrar(e.message,'erro');}finally{if(btn){btn.disabled=false;btn.textContent='Salvar professor';}}}
...
async function sincronizarProfessoresSupabase(){
  const botao=$('#btnSincronizarSupabase');
  if(!botao||botao.disabled)return;
  if(!window.confirm('Copiar os professores locais para o novo Supabase? A base local continuará sendo a fonte principal nesta etapa.'))return;
  const textoOriginal=botao.textContent;
  botao.disabled=true;
  botao.textContent='Sincronizando...';
  mostrar('Enviando professores para o Supabase. Não feche esta página.','info');
  try{
    const resp=await fetch(`${API}/sincronizar-supabase`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    const payload=await safeJson(resp);
    if(!resp.ok||payload.ok===false)throw new Error(payload.mensagem||payload.erro||`HTTP ${resp.status}`);
    mostrar(`${payload.enviados||0} professor(es) sincronizado(s) com sucesso.`,'sucesso');
  }catch(e){
    mostrar(`Falha na sincronização: ${e.message}`,'erro');
  }finally{
...
$('#cpf')?.addEventListener('input',ev=>{ev.target.value=formatarCpf(ev.target.value);});
