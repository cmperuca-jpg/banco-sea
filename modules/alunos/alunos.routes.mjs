...
import {
  configuracaoSincronizacaoAlunos,
  sincronizarAluno,
  sincronizarAlunos
} from "./alunos-supabase-sync.service.mjs";
...
async function copiarAlunoParaNuvem(req, aluno) {
  if (!aluno || req.usuario?.portal) return null;
  return sincronizarAluno({
    aluno,
...
router.get("/sincronizacao/status", async (_req, res) => {
  res.json({ ok: true, ...configuracaoSincronizacaoAlunos() });
});
...
router.post("/sincronizar-supabase", async (req, res) => {
  try {
    ...
    const resultado = await sincronizarAlunos({
      alunos,
      academiaId: req.usuario?.academiaId,
      token: tokenDaRequisicao(req)
    });
    res.json({ ...resultado, mensagem: `${resultado.enviados} aluno(s) copiado(s) para o Supabase.` });
  } catch (error) {
    erro(res, error, 503);
  }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    let nuvem = null;
    try { nuvem = await copiarAlunoParaNuvem(req, aluno); } catch (syncError) { nuvem = { ok: false, mensagem: syncError.message }; }
    res.status(201).json({ ok: true, aluno, nuvem, mensagem: nuvem?.ok === false ? "Aluno salvo localmente; sincronização com a nuvem ficou pendente." : "Aluno cadastrado com sucesso" });
  } catch (error) {
    erro(res, error, 400);
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    let nuvem = null;
    try { nuvem = await copiarAlunoParaNuvem(req, aluno); } catch (syncError) { nuvem = { ok: false, mensagem: syncError.message }; }
    res.json({ ok: true, aluno, nuvem, mensagem: nuvem?.ok === false ? "Aluno atualizado localmente; sincronização com a nuvem ficou pendente." : "Aluno atualizado com sucesso" });
  } catch (error) {
    erro(res, error, 400);
  }
});
