...
export async function dashboardBiblioteca() {
  ...
  return { ok:true, modulo:'biblioteca-inteligente', versao:'Fusion Sistema 2.7', status:'Online', estatisticas: estatisticas(lista), recentes: lista.slice(-8).reverse(), logs: logs.slice(0, 10) };
}
...
export async function statusBibliotecaInteligente() {
  const lista = (await listarBiblioteca()).map(normalizarRegistro);
  return { ok:true, modulo:'biblioteca-inteligente', versao:'Fusion Sistema 2.7', status:'Online', estatisticas:estatisticas(lista) };
}
