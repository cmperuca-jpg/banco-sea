...
async function executarCicloMotor() {
  ...
  try {
    ...
    if (!templates.length) {
      throw erro("Nenhuma biometria ativa com template válido está cadastrada no Fusion Sistema.", 409);
    }
...
export async function identificarSdk() {
  ...
  if (!lista.length) throw erro("Nenhuma biometria ativa está cadastrada no Fusion Sistema.", 409);
  const sdkOk = await garantirSdkLocal({
    tentativas: SDK_STARTUP_TENTATIVAS,
    intervaloMs: SDK_STARTUP_INTERVALO_MS
  });
