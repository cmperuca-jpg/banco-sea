...
#define AppPublisher "Fusion Sistema"
...
begin
  ...
  ActivationPage := CreateInputQueryPage(wpSelectDir,
    'Conectar ao Fusion Sistema',
    'Informe o código mostrado no painel do Fusion Sistema.',
...
begin
  ...
  if Length(ActivationPage.Values[1]) < 8 then begin
    MsgBox('Digite o código de oito números mostrado no Fusion Sistema.', mbInformation, MB_OK);
    Result := False; exit;
  end;
end;
