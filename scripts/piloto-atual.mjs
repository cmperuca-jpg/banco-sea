...
async function main() {
  ...
  const relatorio = {
    ...
    arquivos,
    bases,
    recomendacao: "Executar piloto sem novas alterações estruturais. Registrar pendências para Fusion Sistema 2.7."
  };

  await fs.writeFile(path.join(LOGS, "piloto-atual.json"), JSON.stringify(relatorio, null, 2), "utf8");

  console.log("Fusion Sistema — Validação do Projeto Piloto");
