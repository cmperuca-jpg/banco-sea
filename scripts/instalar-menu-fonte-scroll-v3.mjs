...
async function backup(p){ if (await exists(p)) await fs.copyFile(p, `${p}.bak-menu-v3-${Date.now()}`); }

const cssPatch = `

/* === Fusion Sistema Menu Global v3: fonte, tamanho e scroll fixos === */
:root{
...
const jsContent = `function fusionMenuAtivo() {
...
css = css.replace(/\/\* === Fusion Sistema Menu Global v3:[\s\S]*?body \.fusion-sidebar \.fusion-sidebar-footer\{[\s\S]*?\}\s*/g, '');
if (!css.includes('Fusion Sistema Menu Global v3')) css += cssPatch;
...
console.log('Menu global v3 instalado: fonte/tamanho padronizados e scroll preservado.');
