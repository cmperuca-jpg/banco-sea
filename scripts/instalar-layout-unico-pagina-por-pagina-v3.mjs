...
const BACKUP_DIR = path.join(ROOT, "data", "backups", `layout-unico-v3-${Date.now()}`);
...
const layoutJs = String.raw`(function () {
  ...
  function titleFromModule(moduleName) {
    ...
    return map[moduleName] || document.title.replace(/^Fusion Sistema\s*-\s*/i, "").replace(/\s*-\s*Fusion Sistema$/i, "") || "Fusion Sistema";
  }
  ...
  function buildMenuHtml() {
    ...
    let html = '<aside class="fusion-sidebar" data-fusion-sidebar><div class="fusion-brand">Fusion Sistema</div><nav class="fusion-menu" aria-label="Menu principal">';
